...
(function (jsPDF) {
    ...
    var themes = {
        ...
        'mary': {
            header: { fillColor: 255, textColor: 0, fontStyle: 'normal', lineWidth: 0.1, fillStyle: 'DF' },
            table: { fillColor: 255, textColor: 0, fontStyle: 'normal', lineWidth: 0.1, fillStyle: 'DF' },            
            header: { fontStyle: 'bold'  },
            body: {}
        },
         'amarillo': {
            table: { fillColor: 255, textColor: 80, fontStyle: 'normal', lineWidth: 0.1, fillStyle: 'DF' },
            header: { textColor: 0, fillColor: [255, 204, 51], rowHeight: 23, fillStyle: 'DF', fontStyle: 'bold' },
            body: {},
            alternateRow: {}
        }
    };
...
var require$$7$1 = Object.freeze({
    ...
    jsPDF.API.autoTableText = function (text, x, y, styles) {
        ...
        var lineCount = 1;
        if (styles.valign === 'middle' || styles.valign === 'bottom' || styles.halign === 'center' || styles.halign === 'justify' || styles.halign === 'right') {
        ...
        if (styles.halign === 'center' || styles.halign === 'right'|| styles.halign === 'justify') {
            ...
            if (lineCount >= 1) {
                for (var iLine = 0; iLine < splittedText.length; iLine++) {
                     if (styles.halign === 'justify') {
                        var totall =107 - splittedText[iLine].length; 
                        ...
                        do{
                            ...
                            totall = totall-1;
                            var count = 0;
                            var res = splittedText[iLine].split(' ').length;
                            var just= splittedText[iLine].split(' ');
                            var total = just[0];
                                for(var i = 1; i<=totall;i++){
                                    total=total+'   '+just[i];                              
                                    if(just[i+1]==undefined){
                                        i=totall;
                                        count = totall;
                                    }
                                    count++;
                                ...
                                if(count<res){
                                for (var j = count+1; j<res;j++){                  
                                    total=total+' '+just[j];
                                }
                                }
                                var xe = total;                     
                    
                    }while(this.getStringUnitWidth(xe)>62);
                }while(this.getStringUnitWidth(xe)<60);
                        splittedText[iLine]= total;
                    }
                    this.text(splittedText[iLine], x, y);
                }
                    else{
                    this.text(splittedText[iLine], x - this.getStringUnitWidth(splittedText[iLine]) * alignSize, y);
                    console.log(splittedText[iLine].length,this.getStringUnitWidth(splittedText[iLine]),alignSize,splittedText[iLine]);
                    }
                    y += fontSize;
                }
                return doc;
