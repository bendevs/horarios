app.controller('usgaveController', function ($scope, $q, $rootScope, $location, $route, $http, Data, sessionService,CONFIG, LogGuardarInfo, DreamFactory, $element, sweet, ngTableParams, $filter, registroLog, filterFilter,FileUploader, fileUpload, $timeout, $sce) {
        ...
        var parametros = {
            ...
            "body":{
                "params": [
                {
                    ...
                    "param_type":"IN","value":"select  count(cas_id)as suma ,cas_datos ->> 'USG_PLACA_VALOR' as modelo from _fr_casos  where cas_datos->>'g_tipo'  ='USG_VHTEM'  AND cas_datos ->>'USG_UO_SOL_VALOR'<>'undefined' and cas_datos->>'USG_PLACA_VALOR'<> 'undefined' group by cas_datos->>'USG_PLACA_VALOR' ORDER BY modelo asc"
                }]
            }
        };
        DreamFactory.api[CONFIG.SERVICE].callStoredProcWithParams(parametros).success(function (response){
            ...
            angular.forEach(jsonData,function(celda, fila){
                $scope.totalcalficaciones = $scope.totalcalficaciones + parseInt(celda.suma);
                doughnutData[i] = {
                    value: celda.suma,
                    color: color[i],
                    highlight: "#1C604E",
    ...
    $scope.apoyo = function() {
        ...
        var parametros = {
            ...
            "body":{
                ...
                {
                    "name":"expression",
                    "param_type":"IN","value":"select count(cas_id)as suma ,cas_datos ->> 'USG_CHOF_VALOR' as chofer from _fr_casos where cas_datos->>'g_tipo' = 'USG_VHTEM' AND cas_datos ->>'USG_UO_SOL_VALOR'<>'undefined' and cas_datos ->>'USG_UO_SOL_VALOR'<>'null' and cas_datos->>'USG_CHOF_VALOR'<>'undefined' group by cas_datos ->>'USG_CHOF_VALOR'" 
                }]
            }
        };
        DreamFactory.api[CONFIG.SERVICE].callStoredProcWithParams(parametros).success(function (response){
