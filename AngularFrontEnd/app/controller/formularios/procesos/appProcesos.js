app.controller('procesosController', function ($scope, $route,$rootScope, DreamFactory, CONFIG,sessionService,ngTableParams,$filter,sweet) {
    ...
    $scope.seleccionaProcesoMapa = function(datos){
        ...
        obj.success(function (response) {
            console.log("ACA ESTA TODO", response);
            $.unblockUI();
...
            },3000);
    ...
    $scope.seleccionaProceso = function(datos){
        $.blockUI();
        console.log('datosProceso: ',datos);
        $scope.panelProcesos = false;
            $scope.panelWorkspace = false;
    ...
    $scope.modificarActividadCargar = function(datosActividad){
        console.log("modificarActividadCargar: ",datosActividad);
        $scope.nodosFormulario = [];
        data = [];
        $scope.tablaNodos.reload();

        nodoTempA = new Array();
        nodoTempO = new Object();
        cont = 0;
        ...
        console.log('datosActividad.macro: ',datosActividad.actderivarmacro);

        if(datosActividad.actderivarmacro == 'PM'){
            datosActividad.actderivarmacro='PM';
        }
        else{
            if(datosActividad.actderivarmacro == 'SM'){
              datosActividad.actderivarmacro = 'SM';
          }
          else{
              if(datosActividad.actderivarmacro=='UO'){
                datosActividad.actderivarmacro = 'UO';
              }
          }
        }
        ...
        $scope.cargarNodosAsignados(datosActividad.actid);

    };
    $scope.eliminarActividadCargar = function(datosActividad){
        console.log("eliminarActividadCargar");
    ...
    $scope.limpiarImp = function(){
        $scope.importar = '';
        $scope.boton="new";
        $scope.desabilitado = false;
        $scope.tituloP="Importar Nuevo Proceso";
    };
    ...
    $scope.eliminarProceso = function(datosProceso){
        var opcion = {};
        $.blockUI();
        if (datosProceso.todo){
          var resOpcion = {
            "procedure_name":"eliminar_proceso",
            "body":{"params": [{"name":"id_proceso","value":datosProceso.procid}]}
          };
          var obj=DreamFactory.api[CONFIG.SERVICE].callStoredProcWithParams(resOpcion);
          obj.success(function (response) {
            $.unblockUI();
    ...
    $scope.adicionarActividad = function(datosActividad){
      console.log('Adicionar actividad:',datosActividad);
        ...
        console.log('macro: ',datosActividad.actderivarmacro);
       ...
       // actividad['act_contador'] = 0;
        ...
        actividad['act_derivacion'] = datosActividad.actderivarmacro;
        ...
        obj.success(function(data){
            console.log(data);
            console.log('datosderivar: ',data.act_derivacion);

           angular.forEach(nodoTempA,function(celda, fila){
                    console.log('celda: ',celda);
                    console.log('fila: ',fila);

                    var parametros = {
                        "procedure_name":"sp_set_derivacion",
                        "body":{
                            "params": [
                                {"name":"idactividad","value": data.act_id},
                                {"name":"idnodo","value": celda.noanodo_id},
                                {"name":"idusuario","value": sessionService.get('IDUSUARIO')},
                                {"name":"tipo_derivacion","value": datosActividad.actderivarmacro}
                            ]
                        }
                    };
                    ...
                    obj.error(function(respuestaNodos){
                    });
            });
    ...
    $scope.modificarActividad = function(datosActividad){
      console.log('modificarActividad: ',datosActividad);
        if(datosActividad.actinicio=='INICIO'){
            datosActividad.actinicio='SI';
            datosActividad.actfinal='NO'
        }
        ...
        //derivación por macro
       console.log('modificarActividadmacro: ',datosActividad.actderivarmacro);
    ...
    $scope.getReglaNegocioFormulario = function(formulario){
        ...
        obj.success(function (response) {
            ...
            angular.forEach(response,function(celda, fila){
                ...
                var regex_1 = new RegExp("'#", flags);
                var regex_2 = new RegExp("#'", flags);
                var reem=celda['rnf_regla'].replace(regex_1,"#");
                objeto['rnf_regla']=reem.replace(regex_2,"#");
                $scope.obtNRformulario[fila]=objeto;
            });
            angular.copy($scope.obtNRformulario, $scope._DataActInicio);
    ...
    $scope.resTablecerDataForms  =   function(){
        $scope._DataActInicio_2 = [];
        angular.copy($scope._DataActInicio, $scope._DataActInicio_2);
        $scope.obtNRformulario   = $scope._DataActInicio_2;
    };
    ...
    $scope.getImpresiones = function(actividad)
    {
        ...
        obj.success(function (response) {
            ...
            angular.forEach(response,function(celda, fila){
                ...
                objeto['subirdoc']=celda['subirdoc'];
    ...
    $scope.modificarFormatosImpresion = function(datosImpresion){
        ...
        console.log("MODIFCAR CHECK:", datosImpresion);
        ...
        var resOpcion = {
            ...
            "body":{
                "params": [{
                    ...
                    },{ "name": "impudprc","value": datosImpresion.subirdoc
                    }
                ]
            }
        };
...
//-------------------------------------------------------------------
       ...
       /*var resCampos = {
        ...
        .success(function (results) {
            ...
            if (results.length>0) {
              ...
              if (results[0].get_json_order !=null) {
                console.log("22222:", results[0].get_json_order);
                ...
                angular.forEach(datos,function(celda, fila){
                    ...
                    objetoCampo.tipo_script = celda['tipo_script'];
    ...
    $scope.adicionarCampos = function(datosCampo){
        ...
        $scope.objCampos.tipo_script = datosCampo.tipo_script;
    ...
    $scope.modificarCampos = function(datosCampo){ console.log(datosCampo);
        ...
        for (var i = 0; i <= datos.length-1; i++) {
            if (i != $scope.indice) {
                ...
                objetoCampo2.tipo_script = datosCampo.tipo_script;
   ...
   $scope.clonarCampos = function(idf){
    ...
    $scope.modificarCamposCargar = function(campos,indice){
        ...
        try{
            campos.orden    =   parseInt(campos.orden);
        }catch(e){}
...
$scope.descargarArchivo= function(contenidoEnBlob, nombreArchivo) {
  ...
  reader.readAsDataURL(contenidoEnBlob);
    ...
    $scope.exportar = function(proceso){
      ...
      var obj=DreamFactory.api[CONFIG.SERVICE].callStoredProcWithParams(parametros);
      obj.success(function (response) {
        $scope.obtArbol = response[0];
        var arr=[];
        arr.push($scope.obtArbol.exportar_proceso);
        var texto= new Blob(arr, {type: 'text/plain'});
        $scope.descargarArchivo(texto, proceso.procodigo+'.txt');
        $.unblockUI();
      })
      obj.error(function(error) {
        $.unblockUI();
    ...
    $scope.importar2 = function(imp){
      $.blockUI();
      if (imp === ''){
        $scope.proc=document.getElementById('output').innerText;
        var resRNformulario = {
          "procedure_name":"importar_proceso",
          "body":{"params": [{"name":"datos","value":$scope.proc},{"name":"id","value":sessionService.get('WS_ID')}]}
        };
        var obj=DreamFactory.api[CONFIG.SERVICE].callStoredProcWithParams(resRNformulario);
        obj.success(function (response) {
          var respuesta = response[0];
          if(respuesta.importar_proceso === 'mary'){
            sweet.show('', 'Proceso Existente', 'error');
          }else{
            sweet.show('', 'Nuevo Proceso Registrado', 'success');
            $scope.getProcesos($scope.wsId);
          }
          $.unblockUI();
        });
        obj.error(function(error) {
          sweet.show('', 'Proceso no insertado', 'error');
          $.unblockUI();
        });
      }
      else{
        $scope.proc=document.getElementById('output').innerText;
        var resRNformulario = {
          "procedure_name":"importar_procesos",
          "body":{"params": [{"name":"datos","value":$scope.proc},{"name":"id","value":sessionService.get('WS_ID')},{"name":"nombre","value":imp.nombre},{"name":"codigo","value":imp.codigo}]}
        };
        var obj=DreamFactory.api[CONFIG.SERVICE].callStoredProcWithParams(resRNformulario);
        obj.success(function (response) {
          var respuesta = response[0];
          if(respuesta.importar_proceso === 'mary'){
            ...
            if(respuesta.importar_proceso === 'codigo'){
              sweet.show('', 'Codigo Existente', 'error');
            }else{
              sweet.show('', 'Nuevo Proceso Registrado', 'success');
              $scope.getProcesos($scope.wsId);
            }
    ...
    var nodoTempA = new Array();
    var nodoTempO = new Object();
    var cont = 0;

    $scope.cargarNodosAsignados = function (actid) {
        var parametros = {
            "procedure_name":"nodo_actividad_lst",
            "body":{
                "params": [
                    {"name":"idactividad","value": actid},
                ]
            }
        };
        var obj = DreamFactory.api[CONFIG.SERVICE].callStoredProcWithParams(parametros);
        obj.success(function (respuestaNodos){

            $scope.nodosFormulario = respuestaNodos;
            var data = respuestaNodos;
            $scope.tablaNodos.reload();
    ...
    $scope.tablaNodos = new ngTableParams({
        page: 1,
        count: 10,
        filter: {},
        sorting: { vtra_id: 'desc' }
    }, {
        total: $scope.nodosFormulario.length,
        getData: function($defer, params) {
            var filteredData = params.filter() ?
            $filter('filter')($scope.nodosFormulario, params.filter()) :
            $scope.nodosFormulario;
            var orderedData = params.sorting() ?
            $filter('orderBy')(filteredData, params.orderBy()) :
            $scope.nodosFormulario;
            params.total($scope.nodosFormulario.length);
            $defer.resolve(orderedData.slice((params.page() - 1) * params.count(), params.page() * params.count()));
        }
    });

    $scope.asignarNodo = function (nodoId, resultado) {

      if(resultado.actderivarmacro=='PM'){
          datosmacro = 'PM';
          resultado.actderivarmacro='PM';
      }else{
          if(resultado.actderivarmacro=='SM'){
          nodoTempA=[];
           cont=0;
            datosmacro = 'SM';
            resultado.actderivarmacro='SM';
          }
          else{
                if(resultado.actderivarmacro=='UO'){
                  resultado.actderivarmacro='UO';
                    datosmacro = 'UO';
                }
          }
      }
      ...
      if(resultado.actid)
      {
          var parametros = {
              ...
              "body":{
                  "params": [
                      {"name":"idactividad","value": resultado.actid},
                      {"name":"idnodo","value": nodoId},
                      {"name":"idusuario","value": sessionService.get('IDUSUARIO')},
                      {"name":"tipo_derivacion","value": resultado.actderivarmacro}
                  ]
              }
          };
          var obj = DreamFactory.api[CONFIG.SERVICE].callStoredProcWithParams(parametros);
          obj.success(function (respuesta){
              $scope.cargarNodosAsignados(resultado.actid);
          })
          obj.error(function(respuestaNodos){
          });
      }
      else
      {
        ...
        var e = document.getElementById("nodoid");
        nodoTempO = new Object();
        nodoTempO.noaid = cont;
        nodoTempO.noanodo_id = nodoId;
        nodoTempO.nodonombre = e.options[e.selectedIndex].text;;
        nodoTempA[cont] = nodoTempO;
        cont = cont + 1;
        $scope.nodosFormulario = nodoTempA;
        var data = nodoTempA;
        $scope.tablaNodos.reload();
        angular.forEach(nodoTempA,function(celda, fila){
            console.log(celda);
            console.log(celda.noanodo_id);
        });

      }
    }

    $scope.eliminarNodoActividad = function (results) {
      console.log(results);
      if(results.noaact_id)
      {
          var parametros = {
              "procedure_name":"sp_baja_nodo_actividad",
              "body":{
                  "params": [
                      {"name":"idnodoactividad","value": results.noaid}
                  ]
              }
          };
          var obj = DreamFactory.api[CONFIG.SERVICE].callStoredProcWithParams(parametros);
          obj.success(function (respuesta){
              $scope.cargarNodosAsignados(results.noaact_id);
          })
          obj.error(function(respuestaNodos){
          });
      }
      ...
      {
        nodoTempA.splice(parseInt(results.noaid), 1);
        console.log('nodo: ',nodoTempA.splice(parseInt(results.noaid), 1));
        cont = cont - 1;
        $scope.nodosFormulario = nodoTempA;
        var data = nodoTempA;
        $scope.tablaNodos.reload();

        console.log('nodoTempA ',nodoTempA);
      }
    ...
    $scope.getlistanodos = function(){
         var resAccesos = {
            "table_name":"ct_nodos",
                            "filter": "nodo_estado='ACTIVO'",
                            "order" : "nodo_nombre"
                            };
            var obj = DreamFactory.api[CONFIG.SERVICE].getRecords(resAccesos);
                obj.success(function(data)
                {
                  $scope.getlistanodo=data.record;
                })
    };
