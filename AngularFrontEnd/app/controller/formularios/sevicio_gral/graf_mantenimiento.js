app.controller('grafica_mantenimiento', function ($scope, $q, $rootScope, $location, $route, $http, Data, sessionService,CONFIG, LogGuardarInfo, DreamFactory, $element, sweet, ngTableParams, $filter, registroLog, filterFilter,FileUploader, fileUpload, $timeout, $sce) {
  $scope.registro = {};
  ...
  $scope.categoria=function(numero,cod,registro){
    ...
    switch ($scope.numero){
      case '1':
      $scope.getRep_servicio_tec(cod,registro);
      break;
      case '2':
      $scope.getRep_infra_unidad(cod,registro);
      break;
      case '3':
      $scope.getRep_tecnicos(cod,registro);    
      break;
    }      
  };
  $scope.getRep_servicio_tec = function(cod,registro){
    ...
    $scope.getRep_infra_unidad = function(cod,registro){
      $scope.fechini=$filter('date')(new Date(registro.FECHA1), 'yyyy-MM-dd');
      $scope.fechfin=$filter('date')(new Date(registro.FECHA2), 'yyyy-MM-dd');
      $scope.TablaCreacion = true;
      var reslocal = {
        "procedure_name":"sp_graf_infra_uni",
        "body":{
          "params": [            
          {
            "name":"fechini",
      ...
      $scope.getRep_tecnicos = function(cod,registro){
        $scope.fechini=$filter('date')(new Date(registro.FECHA1), 'yyyy-MM-dd');
        $scope.fechfin=$filter('date')(new Date(registro.FECHA2), 'yyyy-MM-dd');
        $scope.TablaCreacion = true;
        var reslocal = {
          "procedure_name":"sp_graf_tecnicos",
          "body":{
            "params": [            
            {
              "name":"fechini",
        ...
        $scope.pdf = function(image){
          ...
          var header = function (data) {
            ...
            doc.addImage(USG, 'JPEG', data.settings.margin.left, 35, 530, 60);
            doc.text('MANTENIMIENTO DE INFRAESTRUCTURA', data.settings.margin.left + 140, 110);
          };
          doc.setFontSize(10);
          ...
          switch ($scope.numero){
            case '1':
            doc.setFontSize(11);
            doc.text('Reporte - Grafico Por Tipo de Técnico Solicitado',  180, 125);
            break;
            case '2':
            doc.setFontSize(11);
            doc.text('Reporte - Grafico Por Unidad Solicitante',  190, 125);
            break;
            case '3':
            doc.setFontSize(11);
            doc.text('Reporte -Grafico Por Servicio de Técnicos',  185, 125);  
            break;
          }
          var res = doc.autoTableHtmlToJson(document.getElementById("Tramite"));
          doc.autoTable(res.columns, res.data, {theme: 'mary',startY: 150,beforePageContent: header,headerStyles: {rowHeight: 12, fontSize: 8, valign: 'middle'},bodyStyles: {rowHeight: 12, fontSize: 7, valign: 'middle'},margin: {horizontal: 50, top: 140, bottom: 80},styles: {overflow: 'linebreak'}, pageBreak: 'auto'});
