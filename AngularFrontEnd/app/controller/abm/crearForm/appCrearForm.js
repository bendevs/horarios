app.controller('crearFormController', function ($scope, $q,$route,$rootScope, DreamFactory, CONFIG,sessionService,ngTableParams,$filter,sweet,$timeout) { 
    ...
    arrVariablesUrl = arrVariables[1].split("%7C");
    idProceso = arrVariablesUrl[0];
    sessionService.set('WS_ID', arrVariablesUrl[1]);
    //console.log("tramite a crear", idProceso);
    //$scope.procesoid.actid = idProceso;
    //$scope.adicionarCaso();
    //id = arrVariablesUrl[];
    ...
    $scope.crearProcesoByid = function(proceso){
        var resOpcion = {
            function_name: "procesolst_actini",            
            body:{}   
        };
        DreamFactory.api[CONFIG.SERVICE].callStoredFuncWithParams(resOpcion).success(function (response){
            $scope.obtProcesos=response;
            angular.forEach($scope.obtProcesos,function(val, index){
                if(proceso == val.procid){
                    $scope.procidAuto = val.procid;
                    $scope.actidAuto = val.actid;
                    $scope.procodigoAuto = val.procodigo;
                    $scope.adicionarCasoAutomatico();
                }
            });
        }).error(function(error) {
            $scope.errors["error_rol"] = error;
    ...
    $scope.adicionarCasoAutomatico = function(){        
        if($scope.actidAuto!= null)
        {
            var caso = {};
            var resFormulario = {
                ...
                body:{
                    "params": [
                        {
                            "name": "proid",
                            "value": $scope.procidAuto
                        },{
                            "name": "actid",
                            "value": $scope.actidAuto
                        },{
                            "name": "usr_id",
                            "value": sessionService.get('IDUSUARIO')
                        },{
                            "name": "datos",
                            "value": '{"g_fecha": "'+$scope.fechactual+'", "g_tipo": "'+$scope.procodigoAuto+'"}'
                        },{
                            "name": "procodigo",
                            "value": $scope.procodigoAuto
                        },{
                            "name": "macro_id",
                            "value": null
                        },{
    ...
    $scope.$on('api:ready',function(){
        ...
        //$scope.crearProcesoByid(idProceso);
